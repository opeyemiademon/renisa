...
const contactMessageResolvers = {
  Query: {
    getContactMessages: async (_: any, __: any, context: AuthContext) => {
      requireAdminAuth(context);
      return await ContactMessage.find().sort({ createdAt: -1 });
    },
    getContactMessage: async (_: any, { id }: { id: string }, context: AuthContext) => {
      requireAdminAuth(context);
  ...
  Mutation: {
    submitContactMessage: async (_: any, { data }: { data: any }) => {
    ...
    markContactMessageRead: async (_: any, { id }: { id: string }, context: AuthContext) => {
      requireAdminAuth(context);
