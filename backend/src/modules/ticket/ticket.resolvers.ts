...
const ticketResolvers = {
  Ticket: {
    id: (p: any) => p._id?.toString(),
    member: (p: any) => p.memberId,
  },

  TicketReply: {
    id: (p: any) => p._id?.toString(),
    createdAt: (p: any) => p.createdAt?.toISOString?.() ?? String(p.createdAt),
  },

  Query: {
    getMyTickets: async (_: any, { status }: any, context: AuthContext) => {
      requireMemberAuth(context);
      const filter: any = { memberId: context.member!.id };
    ...
    getTicket: async (_: any, { id }: any, context: AuthContext) => {
    ...
    getAllTickets: async (_: any, { status, memberId, priority }: any, context: AuthContext) => {
  ...
  Mutation: {
    createTicket: async (_: any, { data }: any, context: AuthContext) => {
      requireMemberAuth(context);
      const { subject, description, priority = 'medium', attachments = [] } = data;
      if (attachments.length > 4) throw new Error('Maximum 4 attachments allowed');
    ...
    replyToTicket: async (_: any, { data }: any, context: AuthContext) => {
      requireMemberAuth(context);
      const { ticketId, message, attachments = [] } = data;

      const ticket = await Ticket.findById(ticketId);
      if (!ticket) throw new Error('Ticket not found');
      if (ticket.memberId.toString() !== context.member!.id) throw new Error('Access denied');
      if (ticket.status === 'closed') throw new Error('Cannot reply to a closed ticket');

      const member = await Member.findById(context.member!.id).select('firstName lastName').lean() as any;
      ticket.replies.push({
        authorId: context.member!.id as any,
        authorType: 'member',
      ...
      await createNotification(
        ...
        `${member?.firstName} ${member?.lastName} replied to ticket: ${ticket.subject}`,
        ticketId,
        'Ticket'
      );
    ...
    adminReplyToTicket: async (_: any, { data }: any, context: AuthContext) => {
      requireAdminAuth(context);
      const { ticketId, message, attachments = [] } = data;
      ...
      const admin = await AdminUser.findById(context.admin!.id).select('name username').lean() as any;
      const adminName = admin?.name || admin?.username || 'Admin';

      ticket.replies.push({
        authorId: context.admin!.id as any,
        authorType: 'admin',
      ...
      await createMemberNotification(
        ...
        `/member/tickets/${ticketId}`
      );

      return { success: true, message: 'Reply sent', ticket: populated };
    },

    updateTicketStatus: async (_: any, { id, status }: any, context: AuthContext) => {
      requireAdminAuth(context);
      ...
      await createMemberNotification(
        ...
        `Your ticket "${ticket.subject}" has been marked as ${statusLabel}`,
        `/member/tickets/${id}`
      );
