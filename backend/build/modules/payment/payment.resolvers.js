...
const paymentResolvers = {
    Payment: {
        ...
        paymentType: (parent) => parent.paymentTypeId,
        reference: (parent) => parent.transactionRef,
        method: (parent) => parent.paymentMethod,
    },
    Query: {
        getAllPayments: async (_, { memberId, status, year, paymentTypeId, dateFrom, dateTo, reference }, context) => {
            ...
            if (reference)
                filter.transactionRef = { $regex: reference, $options: 'i' };
            if (dateFrom || dateTo) {
                filter.createdAt = {};
                if (dateFrom)
        ...
        getMemberPayments: async (_, { memberId }, context) => {
    ...
    Mutation: {
        initiatePayment: async (_, { data }, context) => {
            ...
            const amount = data.amount || paymentType.amount;
        ...
        verifyPayment: async (_, { reference }, context) => {
            ...
            try {
                ...
                }, { new: true }).populate('memberId', 'firstName lastName email memberNumber')
