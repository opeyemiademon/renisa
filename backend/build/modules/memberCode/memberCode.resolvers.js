...
const memberCodeResolvers = {
    Query: {
        getAllMemberCodes: async (_, { batchName, isUsed }, context) => {
        ...
        getMemberCode: async (_, { id }, context) => {
            requireAdminAuth(context);
    ...
    Mutation: {
        ...
        deleteMemberCode: async (_, { id }, context) => {
            requireAdminAuth(context);
            const code = await MemberCode.findById(id);
            if (!code)
                throw new Error('Member code not found');
            if (code.isUsed)
                throw new Error('Cannot delete a used member code');
...
//# sourceMappingURL=memberCode.resolvers.js.map