...
const videoResolvers = {
    Query: {
        getAllVideos: async (_, { status, category }) => {
            const filter = {};
            if (status)
        ...
        getVideoBySlug: async (_, { slug }) => {
            const doc = await Video.findOneAndUpdate({ slug, status: 'published' }, { $inc: { views: 1 } }, { new: true });
            return doc;
        },
    ...
    Mutation: {
        createVideo: async (_, { data }, context) => {
        ...
        updateVideo: async (_, { id, data }, context) => {
            requireAdminAuth(context);
        ...
        publishVideo: async (_, { id }, context) => {
            requireAdminAuth(context);
            const video = await Video.findByIdAndUpdate(id, { status: 'published', publishedAt: new Date() }, { new: true });
            if (!video)
                throw new Error('Video not found');
            return { success: true, message: 'Video published', data: video };
        },
        archiveVideo: async (_, { id }, context) => {
            requireAdminAuth(context);
...
//# sourceMappingURL=video.resolvers.js.map