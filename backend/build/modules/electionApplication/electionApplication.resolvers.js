...
const populate = (q) => q.populate('electionId')
    .populate('positionId')
    .populate('memberId')
    .populate('approvedBy')
    .populate('rejectedBy');
const electionApplicationResolvers = {
    Query: {
        getElectionApplications: async (_, { electionId }, context) => {
            requireAdminAuth(context);
            return await populate(ElectionApplication.find({ electionId }).sort({ createdAt: -1 }));
        },
        getMyApplications: async (_, __, context) => {
            requireMemberAuth(context);
            return await populate(ElectionApplication.find({ memberId: context.member.id }).sort({ createdAt: -1 }));
        },
        getMyApplicationForElection: async (_, { electionId }, context) => {
            requireMemberAuth(context);
            return await populate(ElectionApplication.findOne({ electionId, memberId: context.member.id }));
        },
        getApplicationsByPosition: async (_, { positionId }, context) => {
            requireMemberAuth(context);
            return await populate(ElectionApplication.find({ positionId, status: 'approved' }).sort({ createdAt: -1 }));
        },
        getPendingApplications: async (_, { electionId }, context) => {
            requireAdminAuth(context);
            const filter = { status: 'pending', paymentStatus: 'paid' };
            if (electionId)
                filter.electionId = electionId;
            return await populate(ElectionApplication.find(filter).sort({ createdAt: -1 }));
        },
    },
    Mutation: {
        submitApplication: async (_, { data }, context) => {
            requireMemberAuth(context);
            ...
            if (election.status !== 'active')
                throw new Error('This election is not accepting applications');
            ...
            if (photoBase64) {
                try {
                    const fileName = await processBase64Upload(photoBase64, 'election-applications', ALLOWED_IMAGE_TYPES, 'application');
        ...
        updateApplicationPayment: async (_, { applicationId, data }, context) => {
            requireMemberAuth(context);
        ...
        confirmApplicationPaystackPayment: async (_, { applicationId, reference, amount }, context) => {
            requireMemberAuth(context);
            const application = await ElectionApplication.findById(applicationId);
            if (!application)
                throw new Error('Application not found');
        ...
        approveApplication: async (_, { data }, context) => {
            requireAdminAuth(context);
            ...
            application.approvedBy = context.admin.id;
            ...
            const positionDoc = await ElectoralPosition.findById(application.positionId).lean();
            await createMemberNotification(String(application.memberId), 'election', 'Candidacy approved', `Your application for ${positionDoc?.title || 'the position'} has been approved. You are now on the ballot.`, '/member/elections');
            const populated = await populate(ElectionApplication.findById(applicationId));
            return { success: true, message: 'Application approved successfully', data: populated };
        },
        rejectApplication: async (_, { data }, context) => {
            requireAdminAuth(context);
            ...
            application.rejectedBy = context.admin.id;
            ...
            const positionDoc = await ElectoralPosition.findById(application.positionId).lean();
            await createMemberNotification(String(application.memberId), 'election', 'Candidacy not approved', rejectionReason
                ? `Your application for ${positionDoc?.title || 'the position'} was rejected: ${rejectionReason}`
                : `Your application for ${positionDoc?.title || 'the position'} was not approved. You may apply for another position in this election.`, '/member/elections');
            const populated = await populate(ElectionApplication.findById(applicationId));
            return { success: true, message: 'Application rejected', data: populated };
        },
        deleteApplication: async (_, { applicationId }, context) => {
            requireMemberAuth(context);
            const application = await ElectionApplication.findById(applicationId);
            if (!application)
                throw new Error('Application not found');
...
//# sourceMappingURL=electionApplication.resolvers.js.map