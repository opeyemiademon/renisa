...
const donationResolvers = {
    Donation: {
        ...
        donationType: (parent) => parent.donationTypeId,
        invoice: (parent) => parent.invoiceId,
        isAcknowledged: (parent) => parent.status === 'acknowledged',
        isMonetary: (parent) => parent.donationMode === 'monetary',
    ...
    Query: {
        getAllDonations: async (_, { status, donationMode }, context) => {
            ...
            if (donationMode)
                filter.donationMode = donationMode;
        ...
        getDonation: async (_, { id }, context) => {
            requireAdminAuth(context);
    ...
    Mutation: {
        ...
        acknowledgeDonation: async (_, { id, adminNotes }, context) => {
            requireAdminAuth(context);
            const updated = await Donation.findByIdAndUpdate(id, { status: 'acknowledged', acknowledgedBy: context.admin.id, adminNotes }, { new: true });
            if (!updated)
                throw new Error('Donation not found');
...
//# sourceMappingURL=donation.resolvers.js.map