  ...
  const mutation = useMutation({
    mutationFn: () =>
      createElection({
        ...
        eligibilityMinYears: parseInt(form.eligibilityMinYears, 10),
        requiresDuesPayment: form.requiresDuesPayment,
        positions: positions
          .filter((p) => p.title.trim())
          .map((p) => ({
            title: p.title,
            description: p.description || undefined,
            maxCandidates: parseInt(p.maxCandidates, 10),
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <Input
          ...
          placeholder="e.g. 2025 General Elections"
          value={form.title}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <Input
            label="Min. Years of Membership"
            type="number"
            value={form.eligibilityMinYears}
            onChange={(e) => setField('eligibilityMinYears', e.target.value)}
          />
        </div>
        <div className="flex items-center gap-3">
          <input
          ...
          />
          <label htmlFor="requiresDues" className="text-sm font-medium text-gray-700">
            Requires dues payment to vote
          </label>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" iconLeft={<Plus className="w-4 h-4" />} onClick={addPosition}>
            Add Position
          </Button>
        </div>
        {positions.map((pos, i) => (
          <div key={i} className="border border-gray-200 rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {positions.length > 1 && (
                <button type="button" onClick={() => removePosition(i)} className="text-red-400 hover:text-red-600">
      ...
      <div className="flex gap-3">
        ...
        <Button onClick={() => mutation.mutate()} loading={mutation.isPending} disabled={!form.title}>
          Create Election
        </Button>
      </div>
    </div>
