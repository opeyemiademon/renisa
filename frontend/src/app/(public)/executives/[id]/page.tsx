...
export default function ExecutiveDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: apiExecutives } = useQuery({
    queryKey: ['executives'],
    queryFn: getAllExecutives,
  })

  const allExecutives = apiExecutives && apiExecutives.length > 0 ? apiExecutives : SAMPLE_EXECUTIVES
  const exec = allExecutives.find((e) => e.id === id)
  ...
  const photo = exec.photo ? (exec.photo.startsWith('http') ? exec.photo : buildImageUrl(exec.photo)) : null
              ...
              <div className="w-40 h-40 md:w-48 md:h-48 rounded-full p-1 bg-gradient-to-br from-[#EBD279] to-[#d4a017] shadow-2xl">
                <div className="w-full h-full rounded-full p-1 bg-[#0d4a25]">
                  <div className="w-full h-full rounded-full overflow-hidden">
                    {photo ? (
                      <img src={photo} alt={exec.name} className="w-full h-full object-cover" />
                    ) : (
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-3 gap-10">
            ...
            <div className="md:col-span-2">
              ...
              {(exec as any).bio ? (
                <p className="text-gray-600 leading-relaxed text-lg">{(exec as any).bio}</p>
              ) : (
                ...
                <dl className="space-y-3">
                  ...
                  {exec.sport && (
                    <div>
                      ...
                      <dd className="text-sm font-semibold text-gray-900 mt-0.5">{exec.sport}</dd>
                    </div>
                  )}
                  {exec.tenure && (
                    <div>
