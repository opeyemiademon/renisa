...
export default function AlumniProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/alumni" className="text-[#1a6b3a] font-medium hover:underline">
            ← Back to Community
          </Link>
        </div>
      </div>
  ...
  const photo = person.profilePicture
    ? (person.profilePicture.startsWith('http') ? person.profilePicture : buildImageUrl(person.profilePicture))
    : null
  const fullName = `${person.firstName}${person.middleName ? ' ' + person.middleName : ''} ${person.lastName}`
  const isAlumni = person.isAlumni ?? true
                  ...
                  <div className="w-full h-full rounded-full overflow-hidden">
                    {photo ? (
                      ...
                      <div className="w-full h-full bg-[#1a6b3a] flex items-center justify-center">
                        <span className="text-white text-5xl font-bold">{person.firstName[0]}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {person.bio ? (
                ...
                <div className="bg-gray-50 rounded-2xl border border-gray-200 p-8 text-center">
                  ...
                  <p className="text-gray-500 font-medium">
                    {person.firstName} is a proud member of the RENISA community.
                  </p>
                ...
                <dl className="space-y-3">
                  {person.sport && (
                    <div>
                      ...
                      <dd className="text-sm font-semibold text-gray-900 mt-0.5">{person.sport}</dd>
                    </div>
                  )}
                  {person.state && (
                    <div>
                      <dt className="text-xs text-gray-400 uppercase tracking-wide">State</dt>
                      <dd className="text-sm font-semibold text-gray-900 mt-0.5">{person.state}</dd>
                    </div>
                  )}
                  {person.stateOfOrigin && (
                    <div>
                      <dt className="text-xs text-gray-400 uppercase tracking-wide">State of Origin</dt>
                      <dd className="text-sm font-semibold text-gray-900 mt-0.5">{person.stateOfOrigin}</dd>
                    </div>
                  )}
                  {person.gender && (
                    <div>
                      <dt className="text-xs text-gray-400 uppercase tracking-wide">Gender</dt>
                      <dd className="text-sm font-semibold text-gray-900 mt-0.5 capitalize">{person.gender}</dd>
                    </div>
                  )}
                  {person.memberNumber && (
                    <div>
                      <dt className="text-xs text-gray-400 uppercase tracking-wide">Member Number</dt>
                      <dd className="text-sm font-semibold text-[#1a6b3a] mt-0.5">{person.memberNumber}</dd>
                    </div>
                  )}
                  <div>
